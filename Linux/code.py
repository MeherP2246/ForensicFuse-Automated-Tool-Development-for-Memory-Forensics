...
class Application(tk.Frame):
    ...
    def create_welcome_page(self):
        ...
        self.welcome_lable = tk.Label(self.frame, text="Automated Memory Analysis tool!\n", bg="black", fg="#aaff33", font=("Terminal",30))
        
        self.welcome_lable.grid(row=0, column=1,pady=20)


        self.note_label1 = tk.Label(self.frame, text=" Information ", bg="#b91919", fg="#ccff33",font=("Terminal",25), wraplength=1000, justify="center")
        self.note_label1.grid(row=2, column=1, pady=10)
        ...
        self.welcome_lable = tk.Label(self.frame,text="The automated Memory Analysis tool facilitates the creation of virtual machines on various operating systems such as Windows, Linux, macOS, and Android, allowing users to run potentially harmful scripts within these isolated environments. Subsequently, users can take memory dumps of these virtual machines and analyze them using the Volatility tool for further investigation. ",
            bg="black", fg="#43af1c",font=("Terminal",15),wraplength=1000, justify="left")
        self.welcome_lable.grid(row=3, column=1,pady=30, padx=30)
        ...
        self.welcome_lable.grid(row=4, column=1,pady=10)

         # Button to start VM creation
        self.welcome_button=tk.Button(self.frame, text="start",bg="#aaff33", fg="#f22525" , font=("Terminal",25), command=self.info_page)
        self.welcome_button.grid(row=5, column=1,pady=10)
...
##############################################################################################################################################################################################################
    def info_page(self):
        ...
        for widget in self.frame.winfo_children():
            widget.destroy()
        self.note_labe = tk.Label(self.frame, text="  Information " , bg="#b91919", fg="#ccff33",font=("Terminal",25), wraplength=1000, justify="center")
        self.note_labe.grid(row=0, column=1,columnspan=3,pady=30)
   

        self.note_label = tk.Label(self.frame,text="Before you start creating your Virtual Machine make sure that you have installed Virtual Box setup in your Host Machine.\n\nIf not then please click on below buttons as per your host machine and see the process download the Virtual box.\n\nAnd If you already have Virtual Box and Virtual machine and want to analye it please proceed with next step ",bg="black", fg="#ccff33",font=("Terminal",15), wraplength=1100, justify="left")
        self.note_label.grid(row=2, column=1,columnspan=3, pady=10)
...
##############################################################################################################################################################################################################
    def iso_Download(self):
        ...
        self.os_version_label = tk.Label(self.frame, text="\nProcess 1 : Download The Image File\n",bg="#ffecce", fg="#b21e1e", wraplength=1000, font=("Terminal",20))
        self.os_version_label.grid(row=0, column=1, columnspan=3,sticky='ew', pady=15, padx=60)

        self.os_version_label = tk.Label(self.frame, text="\nBefore creating your virtual machine, let's download the image file (.iso) file as per your VM\n\n",bg="black", fg="#9cb344", wraplength=1000,justify="center", font=("Terminal",15))
        self.os_version_label.grid(row=1, column=1, sticky='we',columnspan=3, pady=5,padx=60)
       
        # OS Type Selection
        self.create_vmpage1 =tk.Label(self.frame, text="Setp 1: Choose your operating System(OS):", bg="black", fg="#9cb344", justify= "left",wraplength=500,font=("Terminal",12)).grid(row=2, column=0, columnspan=3,padx=15, pady=5, sticky='w')
        ...
        self.os_type_dropdown.grid(row=2, column=1, sticky='e',pady=15, padx=40,columnspan=3)
        self.os_type_dropdown.bind("<<ComboboxSelected>>", self.update_os_version_options)

        # OS Version Selection
        self.os_version_label = tk.Label(self.frame, text="Step 2: Choose OS Version: ",bg="black", wraplength=1000, fg="#9cb344", font=("Terminal",12))
        self.os_version_label.grid(row=6, column=1, sticky='w', columnspan=3,padx=15, pady=5)
        ...
        self.os_version_dropdown.grid(row=6, column=1,sticky='e',pady=15, padx=40,columnspan=3)

 
        # ISO file download
        self.os_version_label = tk.Label(self.frame, text="Setp 3: Please click on Download button to Download the .iso file to create Vm ",bg="black", fg="#9cb344",wraplength=500, justify="left", font=("Terminal",12))
        self.os_version_label.grid(row=9, column=1, sticky='w',columnspan=3,padx=15, pady=5)
        self.create_vm_button = tk.Button(self.frame, text="  Download  ", bg="#ccff33", fg="black",justify="right",font=("Terminal",20), command=self.download_iso)
        self.create_vm_button.grid(row=9, column=3, sticky='w',columnspan=4, pady=10,padx=40)
        
        self.note_labe = tk.Label(self.frame, text="\n" , bg="black",font=("Terminal",10))
        self.note_labe.grid(row=10, column=2,columnspan=3,pady=5,padx=5)

        self.note_labe = tk.Label(self.frame, text="  Note " , bg="#b91919", fg="#ccff33",font=("Terminal",15), wraplength=300, justify="left")
        self.note_labe.grid(row=11, column=2,columnspan=3,pady=5,padx=5)
        
        self.label = tk.Label(self.frame, text="1.If you want to create VM using other .iso files please installed your required  .iso file and use it for creating VM.\n\n2. If you are facing issue while downloading the above .iso files please installed it externally from authorized platform.", wraplength=1200, justify="left",bg="black", fg="#b21e1e", font=("Terminal",10))
        self.label.grid(row=12, column=2, sticky='w', columnspan=3,pady=5,padx=20)
        

        # Next Button (initially hidden)
        self.next_button = tk.Button(self.frame, text="  Next  ",bg="#ccff33", fg="#b21e1e", font=("Terminal",15), command=self.android_genymotion)
        self.next_button.grid(row=15, column=3, sticky='e',columnspan=3, padx=10)

        # Back Button
        self.back_button = tk.Button(self.frame, text="  Back  ", bg="#ccff33", fg="#b21e1e", font=("Terminal",15),command=self.info_page)
        self.back_button.grid(row=15, column=0, sticky='w',columnspan=3, padx=10) 
    ...
    def update_os_version_options(self, event):
        os_versions = {
            ...
            "MAC_OS": ["macOS_Ventura", "macOS_Big_Sur", "macOS_Sonoma"],
            "Android": ["genymotion"]
        }
    ...
    def download_iso(self):
        ...
        os_version_to_iso = {
            ...
            "MAC_OS": {
                "macOS_Ventura": r"https://www.mediafire.com/file/dcji26zay7s3p8r/macOS%20Ventura%20ISO%20for%20VM%20by%20techrechard.com.iso/file#",
                "macOS_Big_Sur": r"https://download2392.mediafire.com/zt0oedepncmgqj7Kq3FpLWtQB7jX7CUrFAfrVdXp1F93HvgfyBRXJRmPYwlguoeNza95jwYqEmiCUmUXfyZkgqyNP9Z_INF1InUi2c7rIvo4wRlsuyH6Cyo_2t3d-CkUlf9sWGDqU1M03GqDT2KEyzl_WJ5nPWtH1LVKaXipsjMP/irna8hwybkhl41f/BigSur13-3-1.iso",
                "macOS_Sonoma": r"https://www.mediafire.com/file/vku90kjifs1fmu0/macOS%20Sonoma%20ISO%20by%20techrechard.com.iso/file#"
            },
...
############################################################################################################################################################################################################## 
    def android_genymotion(self):
        ...
        self.note_labe.grid(row=0, column=1,pady=30,padx=5)
        ...
        self.note_label = tk.Label(self.frame,text="For android once you download the genymotion app you have to access it and need to select your android platform as per your need.\n\nPlease click on below button to see the step by step process"
                                   , bg="black", fg="#ccff33",font=("Terminal",18), wraplength=1000, justify="left")
        self.note_label.grid(row=2, column=1, pady=10)
        

        self.browse_button = tk.Button(self.frame, text="\n  Sample Document  \n",bg="#84ac28", fg="black", font=("Terminal",20), command=self.Sample_Document)
        self.browse_button.grid(row=3, column=1, sticky="w", pady=10, padx=300)
        ...
        # Next Button (initially hidden)
        self.next_button = tk.Button(self.frame, text="  Next  ",bg="#ccff33", fg="#b21e1e", font=("Terminal",15), command=self.create_vm_page)
        self.next_button.grid(row=15, column=3,sticky="e",pady=10)

        # Back Button
        self.back_button = tk.Button(self.frame, text="  Back  ", bg="#ccff33", fg="#b21e1e", font=("Terminal",15),command=self.iso_Download)
        self.back_button.grid(row=15, column=0, sticky='w',pady=10)  
...
##############################################################################################################################################################################################################
    def create_vm_page(self):
        ...
        self.os_version_label = tk.Label(self.frame, text="\n  Process 2: Creation of Virtual Machine.  \n ",bg="#ffecce", fg="#b21e1e", wraplength=1200, font=("Terminal",20))
        self.os_version_label.grid(row=0, column=2, columnspan=5,sticky='ew',  pady=20, padx=5)

        self.os_version_label = tk.Label(self.frame, text="Once you donload the image file i.e (.iso file), please follow the below steps\n",bg="black", fg="#9cb344", wraplength=1200,justify="center", font=("Terminal",18))
        self.os_version_label.grid(row=1, column=1, sticky='we',columnspan=5, pady=15,padx=0)
        
        # Chose .ISO file 
        self.create_isopage =tk.Label(self.frame, text="Step 1: Please select path to where you downloaded .iso file",bg="black", fg="#9cb344", wraplength=350,font=("Terminal",10),justify="left")
        self.create_isopage.grid(row=2, column=1,  columnspan=4,pady=10,padx=0,sticky='w')
        self.iso_path_var = tk.StringVar()
        self.ISO_file_entry = tk.Entry(self.frame,bg="#fff2cc", fg="#b21e1e",font=("Terminal",10),textvariable=self.iso_path_var, highlightthickness=5,width=55,justify="left")
        self.ISO_file_entry.grid(row=2, column=2, sticky="e",columnspan=3,pady=10,padx=30)
        self.browse_button = tk.Button(self.frame, text="Browse",bg="#84ac28", fg="black", font=("Terminal",18), command=self.browse_iso)
        self.browse_button.grid(row=2, column=3, sticky="e",columnspan=4, pady=10,padx=0)
        
        # VM Folder Path
        self.create_vmpage =tk.Label(self.frame, text="Step 2:Select Path where you want to store your VM, otherwise it will store in default path.\nNOTE: You can skip this step",bg="black", fg="#9cb344", wraplength=350, font=("Terminal",10),justify="left")
        self.create_vmpage.grid(row=4, column=1,columnspan=4,pady=10,padx=0,sticky='w')
        self.vm_folder_path_var = tk.StringVar()
        self.vm_folder_entry = tk.Entry(self.frame,bg="#fff2cc", fg="#b21e1e",font=("Terminal",10),textvariable=self.vm_folder_path_var, highlightthickness=5, width=55, justify="left")
        self.vm_folder_entry.grid(row=4, column=2 ,sticky="e",columnspan=3,pady=10,padx=30)
        ...
        self.browse_button.grid(row=4, column=3, sticky="e",columnspan=4, pady=10,padx=0)
        
       
        # Create VM Button
        self.os_version_label = tk.Label(self.frame, text="Setp 3: Now Click on Create Vm button",bg="black", fg="#9cb344",wraplength=350, justify="left", font=("Terminal",10))
        self.os_version_label.grid(row=9, column=1, columnspan=3,pady=10,padx=5,sticky="w")
        self.create_vm_button = tk.Button(self.frame, text="\n  Create VM  \n", bg="#ccff33", width=15, fg="black",justify="left",font=("Terminal",20), command=self.perform_create_vm)
        ...
        # Next Button (initially hidden)
        self.next_button = tk.Button(self.frame, text="  Next  ",bg="#ccff33", fg="#b21e1e", font=("Terminal",15), command=self.Introduction_page)
        self.next_button.grid(row=15, column=3, columnspan=3,sticky="e",pady=10,padx=0)

        # Back Button
        self.back_button = tk.Button(self.frame, text="  Back  ", bg="#ccff33", fg="#b21e1e", font=("Terminal",15),command=self.android_genymotion)
...
##############################################################################################################################################################################################################
    def Introduction_page(self):
        ...
        #Information 
        self.note_label1 = tk.Label(self.frame, text=" Information: ", bg="#b91919", fg="#ccff33",font=("Terminal",25), wraplength=1000, justify="center")
        self.note_label1.grid(row=0, column=1, columnspan=4,pady=10)

        self.note_label1 = tk.Label(self.frame, text="Let's Understand what is memory dump", bg="black", fg="#ccff33",font=("Terminal",20), wraplength=1000, justify="center")
        self.note_label1.grid(row=1, column=1, columnspan=4,pady=10)
        ...
        self.note_label1 = tk.Label(self.frame, text="A memory dump is a snapshot of the contents of a computer's memory at a specific point in time. It captures the state of running processes, system configurations, and data stored in RAM. Memory dumps are commonly used for diagnosing system crashes, debugging software, and analyzing security incidents. They provide valuable insights into the state of a system when an issue occurs, aiding in troubleshooting and resolving problems.\n\nAdditionally, a memory dump enable the safe execution of malicious scripts within isolated virtual machine environments, allowing analysts to study their effects on the system without risking damage to the underlying infrastructure. This approach facilitates a deeper understanding of malware behavior, enabling the development of effective countermeasures and proactive defense strategies.\n\n"
                                    , bg="black", fg="#b91919",font=("Terminal",10), wraplength=1100, justify="left")
        self.note_label1.grid(row=2, column=1,columnspan=4, pady=10)
        
        self.note_label1 = tk.Label(self.frame, text=" Note: ", bg="#b91919", fg="#ccff33",font=("Terminal",18), wraplength=1000, justify="center")
        ...
        self.note_label1 = tk.Label(self.frame, text="You can execute a malicious script within your virtual machine to observe its impact on the system. Afterward, utilizing tools like Volatility, you can analyze the effects of the script on the virtual machine.\n\nTo view a few examples, click on the Reference Script button. Ensure that when installing or running any script, you do so within the virtual machine and not on your host machine."
                                    , bg="black", fg="#b91919",font=("Terminal",10), wraplength=1100, justify="left")
        self.note_label1.grid(row=5, column=1,columnspan=4)


        # Back Button
        self.back_button = tk.Button(self.frame, text="\n  Reference Script  \n", bg="#ccff33", fg="#b21e1e", font=("Terminal",15),justify="right",command=self.Reference_Script)
        self.back_button.grid(row=7, column=2, sticky='e' )

        
        # Back Button
        self.back_button = tk.Button(self.frame, text="  Back  ", bg="#ccff33", fg="#b21e1e", font=("Terminal",15),justify="right",command=self.create_vm_page)
        self.back_button.grid(row=10, column=0, sticky='ew' ,pady=10)

        # Next Button (initially hidden)
        self.next_button = tk.Button(self.frame, text="  Next  ", bg="#ccff33", fg="#b21e1e", font=("Terminal",15),justify="right", command=self.create_memory_dump_page)
        self.next_button.grid(row=10, column=4, sticky='e', pady=10)
...
##############################################################################################################################################################################################################
    ...
    def create_memory_dump_page(self):
        ...
        # Available VMs
        self.available_vms_label = tk.Label(self.frame, text="\nProcess 3 : Click on Refresh button to see \n\nAvailable Virtual Machines in your Virtual box:\n",bg="#ffecce", fg="#b21e1e", wraplength=1200, font=("Terminal",15))
        self.available_vms_label.grid(row=0, column=1, sticky='ew',columnspan=5,padx=5 ,pady=10)
        self.available_vms_listbox = tk.Listbox(self.frame, height=11, width=70 ,bg="#fffdf4", fg="black",font=("Terminal",10))
        self.available_vms_listbox.grid(row=1, column=1, columnspan=5, sticky='e', padx=200, pady=15)

        # Refresh Button
        self.refresh_button = tk.Button(self.frame, text="\n Refresh \n",bg="#ccff33", fg="#b21e1e", font=("Terminal",15),justify="right", command=self.refresh_available_vms)
        self.refresh_button.grid(row=1, column=4, sticky='e', columnspan=5, padx=10, pady=0)

        #Path to save Memory dump
        self.create_vmpage =tk.Label(self.frame, text="Select Path where you want to store your Memory dump file :",bg="black", fg="#9cb344", wraplength=300, font=("Terminal",10),justify="right")
        self.create_vmpage.grid(row=2, column=0, columnspan=3,sticky='w', padx=10, pady=0)
        self.vm_folder_path = tk.StringVar()
        self.vm_folder_entry = tk.Entry(self.frame,bg="#fff2cc", fg="#b21e1e",font=("Terminal",10),textvariable=self.vm_folder_path, width=70, highlightthickness=5, justify="left")
        self.vm_folder_entry.grid(row=2, column=2 ,columnspan=3, sticky="w", padx=10, pady=5)
        self.browse_button = tk.Button(self.frame, text=" Browse ",bg="#84ac28", fg="black", font=("Terminal",15), command=self.browse_folder)
        self.browse_button.grid(row=2, column=4, sticky="e",columnspan=5, padx=10, pady=5)


        self.note_labe = tk.Label(self.frame, text="Note: Make sure your VM is ON while taking memory dump" , bg="black", fg="#b91919",font=("Terminal",12), wraplength=700, justify="left")
        self.note_labe.grid(row=3, column=2,columnspan=3, sticky="we" ,padx=5, pady=5)

        # Snapshot Button
        self.snapshot_button = tk.Button(self.frame, text="Take Memory Dump",bg="#ccff33", fg="#b21e1e", font=("Terminal",15),justify="center", command=self.take_dump)
        ...
        # Back Button
        self.back_button = tk.Button(self.frame, text="  Back  ", bg="#ccff33", fg="#b21e1e", font=("Terminal",15),justify="right",command=self.Introduction_page)
        self.back_button.grid(row=14, column=0, sticky='w' ,pady=(10, 0))

        # Next Button (initially hidden)
        self.next_button = tk.Button(self.frame, text="  Next  ", bg="#ccff33", fg="#b21e1e", font=("Terminal",15),justify="right", command=self.create_Volatility_page)
...
##############################################################################################################################################################################################################
    ...
    def take_dump(self):
        ...
        vbox_manage_path = r"/usr/bin/vboxmanage"
        try:
            # Generate a random 5-letter filename
...
##############################################################################################################################################################################################################
    ...
    # Clear the current view
        ...
        #Information 
        self.note_label1 = tk.Label(self.frame, text=" Information: ", bg="#b91919", fg="#ccff33",font=("Terminal",25), wraplength=1300, justify="center")
        self.note_label1.grid(row=0, column=1, columnspan=4,pady=10)

        # Add widgets for the Docker installation page
        self.Volatility_label=tk.Label(self.frame, text="Let's Analyze the memory dump ",bg="black", fg="#43af1c",wraplength=1000, font=("Terminal",20))
        self.Volatility_label.grid(row=1, column=1, sticky='ew',padx=10, pady=10)

        self.Volatility_label2=tk.Label(self.frame, text="To Anlayze the memory dump we can use volatality2 or Volatility3.",bg="#fff2cc", fg="#b80808",wraplength=1000, font=("Terminal",15))
        self.Volatility_label2.grid(row=2, column=1, sticky='ew', padx=10, pady=5)
        self.Volatility_label3=tk.Label(self.frame, text="Before starts let's understand what is Volatality and \nhow we can use it to analyze any memory dump file:\n  ", bg="black", fg="#ffff11",wraplength=1300, font=("Terminal",18),justify="center")
        self.Volatility_label3.grid(row=3, column=1,padx=5, pady=5,sticky='ew')
        ...
        self.Volatility_label4=tk.Label(self.frame, text="1.Volatility is a critical tool in cybersecurity and digital forensics specializing in the analysis of volatile memory (RAM) to uncover insightsinto system state malware behavior and security incidents.\n", 
                                    bg="black", fg="#ccff33", font=("Terminal",10),wraplength=1000, justify="left")
        ...
        self.Volatility_label5=tk.Label(self.frame, text="2.By examining processes network connections and system artifacts in memory Volatility aids in malware detection incidentresponse forensic investigations threat hunting and proactive defense.\n", 
                                    bg="black", fg="#ccff33", wraplength=1000,font=("Terminal",10), justify="left")
        ...
        self.Volatility_label6= tk.Label(self.frame, text=" NOTE: ", bg="#b91919", fg="#ccff33",font=("Terminal",15), wraplength=1300, justify="center")
        ...
        self.Volatility_label7=tk.Label(self.frame, text="Before you dive into Volatility, make sure docker or any platform that supports volatility tool is Installed! \n\nIf you have trouble installing docker make sure you follow our guide on next page!", 
                                    bg="black", fg="#b80808",wraplength=1300, font=("Terminal",10), justify="center")
        self.Volatility_label7.grid(row=9, column=1,sticky='w')


        # Add a button to go back to the previous page
        self.Volatility_back=tk.Button(self.frame, text="Back", bg="#ccff33", fg="#b21e1e", font=("Terminal",15),justify="right",command=self.goto_memory_dump_option)
        self.Volatility_back.grid(row=13, column=0,sticky='w' ,pady=(10, 0))

        # Next Button (initially hidden)
        self.next_button = tk.Button(self.frame, text="Next", bg="#ccff33", fg="#b21e1e", font=("Terminal",15),justify="right", command=self.create_docker_page)
...
##############################################################################################################################################################################################################
    ...
    # Clear the current view
        ...
        # Add widgets for the Docker installation page
        self.docker_label2=tk.Label(self.frame, text="\nProcess 4 : Please Select the Docker as per your Host Machine \n",bg="#ffecce", fg="#b21e1e", wraplength=1300, font=("Terminal",25), justify="center")
        self.docker_label2.grid(row=0, column=0, columnspan=4, sticky='ew', padx=10, pady=5)

        self.docker_label2=tk.Label(self.frame, text="NOTE: Host Machine means current operating system of your laptop or system where you will analyse the memory dump",bg="black", fg="#ccff33",wraplength=1000, font=("Terminal", 10), justify="left")
        self.docker_label2.grid(row=4, column=0,  columnspan=4,sticky='ew', padx=10, pady=5)

        # Button for Windows
        self.windows__button=tk.Button(self.frame, text="\nWindows\n", bg="#ccff33", fg="#b21e1e", font=("Terminal",15),justify="left",command=self.windows_docker_selction)
        self.windows__button.grid(row=7, column=1,sticky='we', padx=50)

        # Button for Linux
        self.linux__button=tk.Button(self.frame, text="\nLinux\n", bg="#ccff33", fg="#b21e1e", font=("Terminal",15),justify="left", command=self.linux_docker_selction)
        self.linux__button.grid(row=7, column=2,sticky='ew' ,padx=50)

        # Button for macOS
        self.macos__button=tk.Button(self.frame, text="\nMac-OS\n", bg="#ccff33", fg="#b21e1e", font=("Terminal",15),justify="left",command=self.MacOs_docker_selction)
        self.macos__button.grid(row=15, column=1,sticky='ew' ,padx=50)
        

        
        # Button for macOS
        self.macos__button=tk.Button(self.frame, text="\nDocker Creation\n", bg="#ccff33", fg="#b21e1e", font=("Terminal",15),justify="left",command=self.Docker_creation_sample_file)
        ...
        # Add a button to go back to the previous page
        self.docker_back=tk.Button(self.frame, text="Back", bg="#ccff33", fg="#b21e1e", font=("Terminal",15),justify="right",command=self.create_Volatility_page)
...
########################################################################################################################################################################################################        
    ...
    def windows_docker_selction(self):
        ...
        # Add widgets for the Docker installation page
        self.docker_label=tk.Label(self.frame, text=" Select your mode of \n\nDocker implementation !! \n\n ",bg="black", fg="#ccff33", font=("Terminal",25),justify="center")
        self.docker_label.grid(row=0, column=1, sticky='ew',columnspan=3,padx=10, pady=10)

        self.docker_button=tk.Button(self.frame, text="\n  Automated \n  Docker Installation  \n", bg="#990000", fg="#ccff33",font=("Terminal",15), justify="center",command=self.show_windows_docker_info)
        self.docker_button.grid(row=2, column=1, sticky='ew',padx=10,pady=10)


        self.docker_button=tk.Button(self.frame, text="\n  Manual \n  Docker Installation   \n", bg="#990000", fg="#ccff33",font=("Terminal",15), justify="center",command=self.show_windows_docker_manual_info)
        self.docker_button.grid(row=2, column=2, sticky='ew',padx=10, pady=10)

        self.docker_button=tk.Button(self.frame, text="\n  Linux Profile  \n  creation  \n", bg="#990000", fg="#ccff33",font=("Terminal",15), justify="center",command=self.linux_profile_page)
...
########################################################################################################################################################################################################
    ...
    def show_windows_docker_info(self):
        ...
        # Add widgets for the Docker installation page
        self.docker_label=tk.Label(self.frame, text=" Automated Docker Configuration for Windows",bg="black", fg="#ccff33", font=("Terminal",30),justify="center")
        self.docker_label.grid(row=0, column=1, sticky='ew',padx=5, pady=5,columnspan=3)

        self.docker_label2=tk.Label(self.frame, text="Note : Before start, please check you have Docker Desktop Installed.\n\n ",bg="black", fg="#b91919", font=("Terminal",20),justify="center")
        self.docker_label2.grid(row=1, column=1, sticky='ew',padx=5, pady=5,columnspan=3)

        self.docker_label4=tk.Label(self.frame, text="If NO, then please click on Install Docker Desktop and run it after installation.Once done please select path to memory dump and click on pull docker button. ",bg="black", fg="#ccff33", font=("Terminal",13),wraplength=800 ,justify="left")
        self.docker_label4.grid(row=2, column=1, sticky='w',padx=5, pady=15)

        # Add a button to install Docker Desktop
        self.install_docker_button = tk.Button(self.frame, text="Install\nDockerDesktop", bg="#990000", fg="#ccff33", font=("Terminal",17), command=self.install_and_start_docker)
        self.install_docker_button.grid(row=2, column=2, sticky='w', pady=(10, 15))
        
        self.docker_label3=tk.Label(self.frame, text="If YES, please select Memory Dump which you want to analyze using Volatility",bg="black", fg="#ccff33", font=("Terminal",13),wraplength=800,justify="left")
        self.docker_label3.grid(row=3, column=1, sticky='w',padx=10, pady=10)

        # Add a button to select the memory dump file
        self.select_dump_button = tk.Button(self.frame, text=" Select \n Memory Dump ", bg="#990000", fg="#ccff33", font=("Terminal",17), command=self.select_memory_dump)
        self.select_dump_button.grid(row=3, column=2, sticky='w', pady=(10, 15))

        self.docker_label3=tk.Label(self.frame, text="Once done pull the Docker.",bg="black", fg="#ccff33", wraplength=800,font=("Terminal",13),justify="left")
        self.docker_label3.grid(row=4, column=1, sticky='w',padx=10, pady=10) 


        # Add a button to pull the Docker image and open it in terminal
        self.pull_docker_button = tk.Button(self.frame, text=" Pull \n Docker Image", bg="#990000", fg="#ccff33", font=("Terminal",17), command=self.pull_docker_image)
        self.pull_docker_button.grid(row=4, column=2, sticky='w', pady=(10, 15))
        
        
        self.docker_label3=tk.Label(self.frame, text="\n",bg="black", fg="#b21e1e", font=("Terminal",8),justify="left")
        self.docker_label3.grid(row=5, column=0, sticky='ew',padx=0, pady=5)
        
        self.docker_label3=tk.Label(self.frame, text="Once you pull the dockerclick on next to seethe volatility commands",bg="black", fg="#b21e1e", font=("Terminal",8),wraplength=200,justify="right")
        self.docker_label3.grid(row=7, column=2, sticky='ew',padx=0, pady=0)

        # Next Button (initially hidden)
        self.next_button = tk.Button(self.frame, text=" Next ", bg="#ccff33", fg="#b21e1e", font=("Terminal",15),justify="right", command=self.volatility_cheatsheet)
        self.next_button.grid(row=8, column=2, sticky='ew', pady=(5, 0))

        # Add a button to go back to the previous page
        self.windows_docker_back = tk.Button(self.frame, text="  Back  ", bg="#ccff33", fg="#b21e1e", font=("Terminal",15), justify="right", command=self.windows_docker_selction)
        self.windows_docker_back.grid(row=8, column=0, sticky='w', pady=(10, 5))
...
######################################################################################################################################
    def linux_profile_page(self):
        ...
        # Add widgets for the Docker installation page
        self.docker_label=tk.Label(self.frame, text=" Profile creation for Linux virtual machine !! \n\n ",bg="black", fg="#43af1c", font=("Terminal",30),wraplength=1500,justify="center")
        self.docker_label.grid(row=0, column=1, sticky='ew',columnspan=2,padx=10, pady=10)
        
        # Add widgets for the Docker installation page
        self.docker_label=tk.Label(self.frame, text="process of creating a custom Linux profile for Volatility3, a widely used framework for extracting digital artifacts from volatile memory (RAM) samples. Analyzing memory dumps of Linux systems is a crucial task for digital forensics and incident response. There are two references are given to shows the steps to create a custom profile.",bg="black", fg="#ccff33", font=("Terminal",15),wraplength=1200,justify="center")
        self.docker_label.grid(row=1, column=1, sticky='ew',columnspan=3,padx=10, pady=10)


        self.docker_button=tk.Button(self.frame, text="\n  Document  \n", bg="#990000", fg="#ccff33",font=("Terminal",20), justify="center",command=self.linux_profile_Document)
        self.docker_button.grid(row=2, column=1, sticky='e',padx=10, pady=10)
        self.docker_button=tk.Button(self.frame, text="\n Github Link\n ", bg="#990000", fg="#ccff33",font=("Terminal",20), justify="center",command=self.linux_profile_github)
        self.docker_button.grid(row=2, column=2, sticky='w',padx=10, pady=10)
...
#########################################################################################################################################################################################################
 #########################################################################################################################################################################################################
    def linux_docker_selction(self):
        ...
        self.docker_button=tk.Button(self.frame, text="\n  Automated \n  Docker Installation  \n", bg="#990000", fg="#ccff33",font=("Terminal",15), justify="center",command=self.show_linux_docker_info)
        self.docker_button.grid(row=2, column=1, sticky='ew',padx=10,pady=10)



        self.docker_button=tk.Button(self.frame, text="\n  Manual \n  Docker Installation   \n", bg="#990000", fg="#ccff33",font=("Terminal",15), justify="center",command=self.show_Linux_docker_manual_info)
        self.docker_button.grid(row=2, column=2, sticky='ew',padx=10, pady=10)
        ...
        self.docker_label=tk.Label(self.frame, text="\n\n",bg="black", fg="#ccff33", font=("Terminal",15),justify="center")
        self.docker_label.grid(row=5, column=1, sticky='ew',padx=10, pady=10)

        self.windows_back = tk.Button(self.frame, text="  Back  ", bg="#ccff33", fg="#b21e1e", font=("Terminal",15), justify="right", command=self.create_docker_page)
        self.windows_back.grid(row=10, column=0, sticky='w', pady=(10, 0))
...
##################################################################################################################################################################################################
 ...
 # Clear the current view
        ...
        # Add widgets for the Docker installation page
        self.docker_label=tk.Label(self.frame, text=" Automated Docker Configuration for Linux",bg="black", fg="#ccff33", font=("Terminal",30),justify="center")
        self.docker_label.grid(row=0, column=1, sticky='ew',padx=5, pady=5,columnspan=3)
        ...
        # Add a button to install Docker Desktop
        self.install_docker_button = tk.Button(self.frame, text="Install\nDockerDesktop", bg="#990000", fg="#ccff33", font=("Terminal",17), command=self.install_docker)
        self.install_docker_button.grid(row=2, column=2, sticky='w', pady=(10, 15))
        ...
        # Add a button to select the memory dump file
        self.select_dump_button = tk.Button(self.frame, text=" Select \n Memory Dump ", bg="#990000", fg="#ccff33", font=("Terminal",17), command=self.select_path)
        self.select_dump_button.grid(row=3, column=2, sticky='w', pady=(10, 15))
        ...
        # Next Button (initially hidden)
        self.next_button = tk.Button(self.frame, text=" Next ", bg="#ccff33", fg="#b21e1e", font=("Terminal",15),justify="right", command=self.linux_volatility_cheatsheet)
        self.next_button.grid(row=8, column=2, sticky='ew', pady=(5, 0))

        # Add a button to go back to the previous page
        self.windows_docker_back = tk.Button(self.frame, text="  Back  ", bg="#ccff33", fg="#b21e1e", font=("Terminal",15), justify="right", command=self.linux_docker_selction)
        self.windows_docker_back.grid(row=8, column=0, sticky='w', pady=(10, 5))
...
################################################################################################################################################################################################## 
    def show_Linux_docker_manual_info(self):
        ...
        self.docker_label1=tk.Label(self.frame, text="  Manual Docker Installation  ", wraplength=1500, bg="black", fg="#43af1c",font=("Terminal",30), justify="center")
        self.docker_label1.grid(row=1, column=1, sticky='ew',padx=10, pady=5)

        self.docker_label4=tk.Label(self.frame, text="Please follow the below steps :\n",bg="black", fg="#990000", font=("Terminal",20),justify="center")
        self.docker_label4.grid(row=2, column=1, sticky='ew',padx=10, pady=5)
        ...
        self.docker_label4.grid(row=3, column=1, sticky='w',padx=10, pady=5)

        self.docker_label4=tk.Label(self.frame, text="$ sudo apt install docker-compose",wraplength=1000, bg="black",fg="#990000", font=("Terminal",12),justify="left")
        self.docker_label4.grid(row=4, column=1, sticky='w',padx=10, pady=5)
        ...
        self.docker_label4.grid(row=5, column=1, sticky='w',padx=10, pady=5)

        self.docker_label4=tk.Label(self.frame, text="$ sudo docker run hello-world",wraplength=1000, bg="black",fg="#990000", font=("Terminal",12),justify="left")
        self.docker_label4.grid(row=6, column=1, sticky='w',padx=10, pady=5)

        self.docker_label4=tk.Label(self.frame, text="3.Now pull the docker using :",wraplength=1000, bg="black", fg="#ccff33", font=("Terminal",12),justify="left")
        self.docker_label4.grid(row=7, column=1, sticky='w',padx=10, pady=5)

        self.docker_label4=tk.Label(self.frame, text="$docker pull pmeher/volatility ",wraplength=1000, bg="black",fg="#990000", font=("Terminal",12),justify="left")
        self.docker_label4.grid(row=8, column=1, sticky='w',padx=10, pady=5)

        self.docker_label4=tk.Label(self.frame, text="4.Access your memory dump in docker using :",wraplength=1000, bg="black", fg="#ccff33", font=("Terminal",12),justify="left")
        self.docker_label4.grid(row=9, column=1, sticky='w',padx=10, pady=5)
        
        self.docker_label4=tk.Label(self.frame, text="$docker run -it --name any_random_name -v path_of_your_memory_dump:/app/memory_dump_name pmeher/volatility \n\nExample : $docker run -it --name user -v /media/kali/:/app/memory_dump pmeher/volatility ",bg="black",fg="#990000", font=("Terminal",12),justify="left")
        self.docker_label4.grid(row=10, column=1, sticky='w',padx=10, pady=5)

        self.docker_label4=tk.Label(self.frame, text="6.Click on next to see the volatility commands to analyze the memory dump file \n\n",bg="black", fg="#ccff33", font=("Terminal",15),justify="left")
        self.docker_label4.grid(row=11, column=1, sticky='w',padx=10, pady=5)
...
##################################################################################################################################################################################################
    ...
    def pull_linuxdocker_image(self):
        if self.selected_file_path:
            ...
            sudo_password = tk.simpledialog.askstring("Sudo Password", "Enter your root password:", show='*')
            if sudo_password:
                command = f'echo {sudo_password} | sudo -S {pull_command}'
                subprocess.Popen(["x-terminal-emulator", "-e", f"bash -c '{command}; exec bash'"])
            else:
                messagebox.showwarning("Password Not Entered", "Please enter the sudo password.")
            # Mount selected file into container
            volume = ''.join(random.choice(string.ascii_lowercase) for _ in range(3))
            open_command = f'sudo docker run -it --name {volume} -v {self.selected_file_path}:/app/{file_name} pmeher/volatility'
            subprocess.Popen(["x-terminal-emulator", "-e", f"bash -c '{open_command}; exec bash'"])
        else:
            messagebox.showwarning("File Not Selected", "Please select a memory dump file before pulling the Docker image.")
######################################################################################################################################    
...
##############################################################################################################################################################################################################
    ...
    def MacOs_docker_selction(self):
        ...
        self.docker_button=tk.Button(self.frame, text="\n  Automated \n  Docker Installation  \n", bg="#990000", fg="#ccff33",font=("Terminal",20), justify="center",command=self.show_macos_docker_info)
        self.docker_button.grid(row=2, column=1, sticky='ew',padx=10,pady=10)



        self.docker_button=tk.Button(self.frame, text="\n  Manual \n  Docker Installation   \n", bg="#990000", fg="#ccff33",font=("Terminal",20), justify="center",command=self.show_macos_docker_manual_info)
        self.docker_button.grid(row=2, column=2, sticky='ew',padx=10, pady=10)
        
        
        self.docker_button=tk.Button(self.frame, text="\n  Linux Profile  \n  creation  \n", bg="#990000", fg="#ccff33",font=("Terminal",20), justify="center",command=self.linux_profile_page)
        self.docker_button.grid(row=2, column=3, sticky='ew',padx=10, pady=10)
...
# Add widgets for the Docker installation page
        ...
        # Add a button to install Docker Desktop
        self.install_docker_button = tk.Button(self.frame, text="  Install   \n  Docker  \n  Desktop  ", bg="#990000", fg="#ccff33", font=("Terminal",20), command=self.install_macos_docker)
        ...
        # Add a button to select the memory dump file
        self.select_dump_button = tk.Button(self.frame, text=" Select \n Memory Dump ", bg="#990000", fg="#ccff33", font=("Terminal",20), command=self.select_macos_path)
        ...
        # Add a button to pull the Docker image and open it in terminal
        self.pull_docker_button = tk.Button(self.frame, text="Pull \n Docker Image", bg="#990000", fg="#ccff33", font=("Terminal",20), command=self.pull_macosdocker_image)
...
##############################################################################################################################################################################################################
    def install_macos_docker(self):
        ...
        if sudo_password:
            ...
            command = f'echo {sudo_password} | sudo -S brew update && echo {sudo_password} | sudo -S brew install --cask docker'
            subprocess.call(command, shell=True)
            self.message_label.config(text="Docker installed successfully.")
        else:
            self.message_label.config(text="Installation cancelled.")
    ...
    def pull_macosdocker_image(self):
        ...
        if self.selected_file_path:
            ...
            volume = ''.join(random.choice(string.ascii_lowercase) for _ in range(3))
            # Command to run Docker container on macOS
            open_command = f'docker run -it --name {volume} -v {self.selected_file_path}:/app/{file_name} pmeher/volatility'
            os.system(f"osascript -e 'tell app \"Terminal\" to do script \"{open_command}\"'")
