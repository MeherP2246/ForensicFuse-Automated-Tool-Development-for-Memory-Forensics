...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.master.title("ForensicFuse :Automated Memory Analysis Tool")
...
##############################################################################################################################################################################################################
    ...
    def show_Linux_Vm_install(self):
        ...
        self.note_text = tk.Text(self.frame, bg="black", fg="#b91919", height=22, width=100, font=("Terminal", 12))
        self.note_text.insert("end", instructions)
        self.note_text.config(state="disabled")
        self.note_text.pack(fill="both", expand=True)
    	 # Back Button
...
##############################################################################################################################################################################################################
    def iso_Download(self):
        ...
        # ISO file download
        self.os_version_label = tk.Label(self.frame, text="\nSetp 3: Please click on Download button to Download the .iso file to create Vm ",bg="black", fg="#9cb344",wraplength=500, justify="left", font=("Terminal",15))
        self.os_version_label.grid(row=9, column=1, sticky='w',columnspan=3,pady=25, padx=15)
...
##############################################################################################################################################################################################################
    def create_vm_page(self):
        ...
        self.os_version_label.grid(row=0, column=2, columnspan=5,sticky='ew',  pady=50, padx=5)
        ...
        self.create_isopage.grid(row=2, column=1,  columnspan=4,pady=10,padx=5,sticky='w')
        self.iso_path_var = tk.StringVar()
        self.ISO_file_entry = tk.Entry(self.frame,bg="#fff2cc", fg="#b21e1e",font=("Terminal",15),textvariable=self.iso_path_var, highlightthickness=5,width=55,justify="left")
        self.ISO_file_entry.grid(row=2, column=2, sticky="e",columnspan=3,pady=10,padx=10)
        ...
        self.create_vmpage.grid(row=4, column=1,columnspan=4,pady=10,padx=5,sticky='w')
        self.vm_folder_path_var = tk.StringVar()
        self.vm_folder_entry = tk.Entry(self.frame,bg="#fff2cc", fg="#b21e1e",font=("Terminal",15),textvariable=self.vm_folder_path_var, highlightthickness=5, width=55, justify="left")
        self.vm_folder_entry.grid(row=4, column=2 ,sticky="e",columnspan=3,pady=10,padx=10)
...
##############################################################################################################################################################################################################
    ...
    def create_memory_dump_page(self):
        ...
        self.available_vms_label.grid(row=0, column=1, sticky='ew',columnspan=5,padx=5 ,pady=20)
        ...
        self.create_vmpage.grid(row=2, column=0, columnspan=3,sticky='w', padx=0, pady=0)
        self.vm_folder_path = tk.StringVar()
        self.vm_folder_entry = tk.Entry(self.frame,bg="#fff2cc", fg="#b21e1e",font=("Terminal",15),textvariable=self.vm_folder_path, width=100, highlightthickness=5, justify="left")
...
########################################################################################################################################################################################################        
    ...
    def windows_docker_selction(self):
        ...
        self.docker_button=tk.Button(self.frame, text="\n  Automated \n  Docker Installation  \n", bg="#990000", fg="#ccff33",font=("Terminal",20), justify="center",command=self.show_windows_docker_info)
        self.docker_button.grid(row=2, column=1, sticky='ew',padx=10,pady=10)


        self.docker_button=tk.Button(self.frame, text="\n  Manual \n  Docker Installation   \n", bg="#990000", fg="#ccff33",font=("Terminal",20), justify="center",command=self.show_windows_docker_manual_info)
...
########################################################################################################################################################################################################
    ...
    def show_windows_docker_info(self):
        ...
        self.install_docker_button = tk.Button(self.frame, text="  Install   \n  Docker  \n  Desktop  ", bg="#990000", fg="#ccff33", font=("Terminal",20), command=self.install_and_start_docker)
        ...
        self.select_dump_button = tk.Button(self.frame, text=" Select \n Memory Dump ", bg="#990000", fg="#ccff33", font=("Terminal",20), command=self.select_memory_dump)
        ...
        self.pull_docker_button = tk.Button(self.frame, text="Pull \n Docker Image", bg="#990000", fg="#ccff33", font=("Terminal",20), command=self.pull_docker_image)
...
######################################################################################################################################
    def linux_profile_page(self):
        ...
        self.docker_button.grid(row=3, column=1, sticky='e',padx=10, pady=20)
...
#########################################################################################################################################################################################################
 #########################################################################################################################################################################################################
    def linux_docker_selction(self):
        ...
        self.docker_button=tk.Button(self.frame, text="\n  Automated \n  Docker Installation  \n", bg="#990000", fg="#ccff33",font=("Terminal",20), justify="center",command=self.show_linux_docker_info)
        self.docker_button.grid(row=2, column=1, sticky='ew',padx=10,pady=10)



        self.docker_button=tk.Button(self.frame, text="\n  Manual \n  Docker Installation   \n", bg="#990000", fg="#ccff33",font=("Terminal",20), justify="center",command=self.show_Linux_docker_manual_info)
        self.docker_button.grid(row=2, column=2, sticky='ew',padx=10, pady=10)
...
# Add widgets for the Docker installation page
        ...
        # Add a button to install Docker Desktop
        self.install_docker_button = tk.Button(self.frame, text="  Install   \n  Docker  \n  Desktop  ", bg="#990000", fg="#ccff33", font=("Terminal",20), command=self.install_docker)
        self.install_docker_button.grid(row=3, column=2, sticky='w', pady=10)
        ...
        # Add a button to select the memory dump file
        self.select_dump_button = tk.Button(self.frame, text=" Select \n Memory Dump ", bg="#990000", fg="#ccff33", font=("Terminal",20), command=self.select_path)
        self.select_dump_button.grid(row=5, column=2, sticky='w', pady=(10, 10))
        ...
        # Add a button to pull the Docker image and open it in terminal
        self.pull_docker_button = tk.Button(self.frame, text="Pull \n Docker Image", bg="#990000", fg="#ccff33", font=("Terminal",20), command=self.pull_linuxdocker_image)
        self.pull_docker_button.grid(row=7, column=2, sticky='w', pady=(10, 10))
